        ...
        <tbody id = "tbl_order">
            <?php foreach ($orders as $key => $value) : ?>
                <tr id = tr_<?php echo $value->id; ?>>
                    ...
                    <td id="status_<?php echo $value->id; ?>" style="color: red;">
                        <span>
                            Chờ xác nhận
                        </span>
                    </td>
                    <td style="width: 50px;">
                        <button <?php echo $value->status == 2 ? 'disabled' : "" ?> type="button" id="btnApproval_<?php echo $value->id ?>" value="submit" class="btn  btn-primary" onclick="acceptOrder(<?php echo $value->id; ?>);" >
                            <?php
                            if ($value->status == 2) {
                                echo "Đã xác nhận";
                            } else {
                    ...
                    <td >
                        <a href="<?php echo Uri::base() . 'sale/orderaccepted/detail/' . $value->id; ?>" class="btn btn-primary" >
                            Chi tiết
                        </a>
                    </td>
                    <td>
...
<script>
    ...
    {
        $.ajax({
            url: base_url + "sale/orderaccepted/acceptOrder/",
            type: "POST",
            data: {orderId: orderId},
            async: false,
            success: function () {

//                $("#status_" + orderId).text("Đã chuyển");
...
//                $("#btnCancel_" + orderId).removeAttr('disabled');
    ...
    {
        $.ajax({
            url: base_url + "sale/orderaccepted/orderCancel/",
            type: "POST",
            data: {orderId: orderId},
            async: false,
            success: function () {
...
//                $("#btnApproval_" + orderId).removeAttr('disabled');
    ...
    {
        $.ajax({
            url: base_url + "sale/orderaccepted/deleteOrder/",
            type: "POST",
            data: {orderId: orderId},
            async: false,
            success: function () {
