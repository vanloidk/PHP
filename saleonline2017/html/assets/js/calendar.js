    ...
    Calendar.prototype._update_month_year = function () {
        ...
        $('.cal-month-day, .cal-year-box .span3')
                ...
                .on('click', function (event) {
                    //if($('.list-events', this).length == 0) return;
                    if ($('.list-events a', this).length <= 3)
                        return;
                    //alert($('.list-events a', this).length);
                    if ($(this).children('[data-cal-date]').text() == self.activecell)
                        return;
                    //showEventsList(event, downbox, slider, self);
                    if ($('.list-events .day-event-item:visible', this).length == 3) {
                        $(this).parents('.cal-cell').css('height', 'auto');
                        $('.list-events', this).find('.day-event-item.hidden-event').show();
                        $(this).find('.tick span.more').hide();
                        $(this).find('.tick span.less').show();
                    } else if ($('.list-events .day-event-item:visible', this).length > 3) {
                        $('.list-events', this).find('.day-event-item.hidden-event').hide();
                        $(this).find('.tick span.more').show();
                        $(this).find('.tick span.less').hide();
                        $(this).parents('.cal-before-eventlist').find('.cal-cell').each(function () {
                            $(this).css('height', 'auto');
                        });
                    }

                    var heightChange = $(this).parents('.cal-before-eventlist').height();
                    $(this).parents('.cal-before-eventlist').find('.cal-cell').each(function () {
